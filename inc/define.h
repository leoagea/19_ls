...
/*   Updated: 2025/05/29 20:23:45 by lagea            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#define AFLAG	"\t-a	do not ignore entries starting with .\n"
#define RFLAG \
	"\t-r	reverse order while sorting\n\
\t-R	list subdirectories recursively\n"
#define TFLAG	"\t-t	sort by time, newest first\n"
#define LFLAG	"\t-l	use a long listing format\n"
#define EXTFLAG "\t-@	list extended attributes\n"
#define SIZEFLAG \
	"\t-S	sort by file size, largest first\n\
\t-s	print the allocated size of each file, in blocks\n"
#define FFLAG "\t-f	do not sort, enable -a, disable -ls\n"
#define GFLAG \
	"\t-g	same as -l, but do not list owner\n\
\t-n	same as -l, but list numeric user and group IDs\n"
#define SLASHFLAG "\t-p	add a slash after each directory name\n"
#define ONEEFLAG  "\t-1	list one file per line\n"
#define COMMAFLAG "\t-,	list entries separated by commas\n"
#define MFLAG	  "\t-m	list entries horizontally\n"
#define UFLAG	  "\t-u	sort by last access time instead of last modification time\n"
#define DFLAG	  "\t-d	do not list subdirectories, only the directories themselves\n"
#define HFLAG	  "\t-h	print sizes in human readable format (e.g., 1K, 234M, 2G)\n"
#define EXIT \
	"\nExit status:\n\
\t0  if OK,\n\
